    ...
    send(payload, callback) {
        ...
        const tryProvider = (index) => {
            ...
            provider.send(payload, (err, result) => {
                ...
                if (result) {
                    const a = JSON.stringify(result).toLowerCase();
                    if (a.includes('too many requests') || a.includes('-32005')) {
                        err = new Error(`Provider '${provider.host}' returned an error: ${JSON.stringify(result.error)}`);
                    }
                }
